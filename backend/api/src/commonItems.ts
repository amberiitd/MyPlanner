...
const create = async (request: CrudRequest) => {
    await createItem(`uid:${request.uid}`, `${request.itemType}:${request.data.id}`, {
...
const getAll = async (request: CrudRequest) => {
    const items = await getItems(`uid:${request.uid}`, request.itemType)
...
const deleteOne = async (request: CrudRequest) => {
    await deleteItem(`uid:${request.uid}`, `${request.itemType}:${request.data.id}`)
    return {
        statusCode: 200,
        body: JSON.stringify({
            'message': 'success',
