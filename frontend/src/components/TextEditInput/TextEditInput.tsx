...
const TextEditInput: FC<TextEditInputProps> = (props) =>{
    ...
    return (
        <div ref={compRef} className='w-100'>
            <div className={`p-1 bg-smoke-hover rounded ${props.extraClasses} ${!props.disabled ? 'cursor-pointer': ''} w-inherit`} onClick={()=> {if (!props.disabled) setActive(true)}} hidden={active}>
                {props.value || '-'}
            </div>
            <div className='position-relative' hidden={!active}>
                <div>
                    <input className={`rounded p-1 input-focus ${props.inputClasses?? 'w-100'}`} type="text" value={value} onChange={(e)=> {setValue(e.target.value)}}/>
                </div>
                ...
                >
                    ...
                    <div className='ms-2 shadow-sm  bg-white'>
                        <Button 
                            ...
                            extraClasses='px-1 btn-as-light'
                            handleClick={()=>{
                                setValue(props.value); setActive(false)
                            }}
                        />
                    </div>
                </div>
